...
const TagBtn = ({ catItem, onActivate, onDeactivate, active }: SingleTagProps) => {
    ...
    return (
        <button
            ...
            className={`${active ? 'bg-[#ff0088] border-[#ff0088]' : 'bg-zinc-900 '} inline-block px-4 text-md w-fit py-1 m-1 rounded-full border-2 border-zinc-900 text-zinc-50 hover:border-[#ff0088] dark:border-zinc-600`}>
            {catItem}
        </button>
    )
}
